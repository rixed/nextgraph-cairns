...
| **B-15** | Device capabilities — geolocation first — reaching an app the framework hosts in an iframe | Whoever owns the embedding page (the auth server today); a framework convention about which capabilities a hosted app is granted | None. Every proximity feature is conditional already (§8), so they are absent rather than broken | Zero to build, but it removes S-01's two best cards and S-40's ordering | **Open** — measured at milestone 6: `getCurrentPosition` fails with *"disabled in this document by permissions policy"* before any prompt. One `allow="geolocation"` attribute fixes it, in a page this app does not own. The general question is which device capabilities a hosted app gets, and who decides |
